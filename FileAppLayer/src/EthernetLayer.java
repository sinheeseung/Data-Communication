...
public class EthernetLayer implements BaseLayer {
    ...
    public boolean Send(byte[] input, int length) {
		...
		System.out.println("Send : " + m_sHeader.enet_type[1]);
    ...
    public boolean fileSend(byte[] input, int length) {
		...
		//file 전송인 경우 0x2090으로
		byte[] bytes = ObjToByte(m_sHeader, input, length);
		System.out.println("fileSend : " + m_sHeader.enet_type[1]);
		this.GetUnderLayer().Send(bytes, length + 14);
		return true;
	}
	...
	public synchronized boolean Receive(byte[] input) {
		...
		System.out.println("Receive : "+(byte)m_sHeader.enet_type[1]);
		if(temp_type == 2)
			//ack인 경우 상위 계층에 알림
			this.GetUpperLayer(0).Receive(null);
		if(input[13] == (byte)0x2080) {
			//chatting인 경우(type이 0x2080)
		...
		else if(input[13] == (byte)0x2090)
		{
			//file인 경우(type이 0x2090)
